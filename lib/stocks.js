import { randomHeadline } from './stockNews';
...
export async function updateStockPrices(sb, { force = false } = {}) {
  ...
  for (const stock of stocks) {
    ...
    const { data: recentHistory, error: recentHistErr } = await sb
      .from('stock_price_history')
      .select('price')
      .eq('stock_id', stock.id)
      .order('recorded_at', { ascending: false })
      .limit(MEAN_REVERSION_WINDOW);
    if (recentHistErr) throw recentHistErr;
    const avgPrice = movingAverage((recentHistory || []).map((h) => h.price).reverse());
    ...
    if (Math.random() < AUTO_NEWS_CHANCE) {
      ...
      // 직전 뉴스와 같은 방향이 이어지게 해뒀더니(스티키니스), 호재가 연달아 나오면
      // 끝없이 우상향할 수 있다는 문제가 있었음. 뉴스 방향이 "이미 고평가/저평가 상태를
      // 더 심하게 만드는" 방향이면 효과를 절반으로 줄여서 — 진짜 시장처럼 "이미 많이
      // 반영된" 호재는 예전만큼 안 먹히게(체감 효과 감소) 만듭니다.
      if (avgPrice) {
        const overshoot = (candidatePrice - avgPrice) / avgPrice;
        const overheating =
          (direction === 'up' && overshoot > VALUATION_THRESHOLD) ||
          (direction === 'down' && overshoot < -VALUATION_THRESHOLD);
        if (overheating) {
          candidatePrice = Math.round((stock.price + candidatePrice) / 2);
        }
      }
      nextPrice = Math.max(1, candidatePrice);
