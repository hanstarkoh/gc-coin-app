...
import { randomEarningsHeadline } from './stockEarnings';
...
// 실적발표: 종목마다 다른 주기(대략 한 달, ±나흘 무작위)로 일반 뉴스보다 훨씬 큰 폭의
// 가격 변동이 옵니다. 종목의 "펀더멘털"(성장중/정체/위기)이 결과 확률을 유리하게/불리하게
// 만들 뿐 결과를 확정 짓진 않아요 — 성장중이어도 가끔 "어닝쇼크"가 나야, 펀더멘털 라벨만
// 보고 기계적으로 사고파는 계산 게임이 되지 않습니다.
export const EARNINGS_INTERVAL_MIN_DAYS = 26;
export const EARNINGS_INTERVAL_MAX_DAYS = 34;
export const EARNINGS_PCT_MIN = 15;
export const EARNINGS_PCT_MAX = 35;
export const EARNINGS_UP_PROB = { growing: 0.68, stagnant: 0.5, crisis: 0.32 };
export const STOCK_EARNINGS_KEEP = 10;

export function nextEarningsDate(from = new Date()) {
  const days = EARNINGS_INTERVAL_MIN_DAYS + Math.random() * (EARNINGS_INTERVAL_MAX_DAYS - EARNINGS_INTERVAL_MIN_DAYS);
  return new Date(from.getTime() + days * 24 * 60 * 60 * 1000);
}

// 현실에서도 한 분기 동안 나온 뉴스 흐름(신제품 히트, 계약, 리콜, 소송 등)이 실적 예상치에
// 반영되는 것처럼, 최근 한 달간 이 종목에 뜬 뉴스의 호재/악재 쏠림이 실적 확률을 밀어줍니다.
// 한 달 내내 누가 봐도 좋은 뉴스만 떴다면(newsSkew=+1) 웬만해선 실제로 오르는 게 맞다고
// 느껴져야 해서, 펀더멘털이 나빠도(위기=32%) 뉴스가 충분히 좋으면 50%를 넘길 수 있을 만큼
// 영향력을 크게 잡았습니다(±30%p). 그래도 100%는 안 만들어서(최대 85%) "확실히 반대로
// 갈 가능성"은 항상 남겨두고, 그 반대의 경우엔 헤드라인에 이유(일회성 비용, 환율 등)를
// 붙여서(lib/stockEarnings.js) 왜 뉴스랑 다르게 나왔는지 납득되게 설명합니다.
export const EARNINGS_NEWS_INFLUENCE = 0.3;
export const EARNINGS_NEWS_LOOKBACK_DAYS = 30;
const EARNINGS_UP_PROB_MIN = 0.15;
const EARNINGS_UP_PROB_MAX = 0.85;

export async function fetchNewsSkew(sb, stockId, days = EARNINGS_NEWS_LOOKBACK_DAYS) {
  try {
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
    const { data, error } = await sb.from('stock_news').select('pct').eq('stock_id', stockId).gte('created_at', since);
    if (error || !data || data.length === 0) return 0;
    const up = data.filter((n) => n.pct > 0).length;
    return (up * 2 - data.length) / data.length; // (up - down) / total
  } catch {
    return 0;
  }
}

export function decideEarningsDirection(fundamental, newsSkew = 0) {
  const base = EARNINGS_UP_PROB[fundamental] ?? EARNINGS_UP_PROB.stagnant;
  const upProb = Math.max(EARNINGS_UP_PROB_MIN, Math.min(EARNINGS_UP_PROB_MAX, base + newsSkew * EARNINGS_NEWS_INFLUENCE));
  return Math.random() < upProb ? 'up' : 'down';
}

export function randomEarningsPct(direction) {
  const magnitude = EARNINGS_PCT_MIN + Math.floor(Math.random() * (EARNINGS_PCT_MAX - EARNINGS_PCT_MIN + 1));
  return direction === 'down' ? -magnitude : magnitude;
}

// 관리자가 실적발표일을 직접 정했을 때, 그 날짜가 한 주기(최대 34일)보다 가깝다면
// "이미 이전 주기가 진행 중이었다"는 뜻이니 자연스럽도록 이전 실적발표 내역을 하나
// 과거 날짜로 채워둡니다. 이미 실적 내역이 있는 종목이면 건너뜁니다(중복 방지).
// 현재 시세에는 영향을 주지 않는 순수 기록용입니다.
export async function backfillEarningsIfNeeded(sb, stock, nextEarningsAtIso) {
  try {
    const { data: existing, error: existErr } = await sb.from('stock_earnings').select('id').eq('stock_id', stock.id).limit(1);
    if (existErr || (existing && existing.length > 0)) return;

    const nextEarningsAt = new Date(nextEarningsAtIso);
    const daysUntil = (nextEarningsAt.getTime() - Date.now()) / (24 * 60 * 60 * 1000);
    if (daysUntil >= EARNINGS_INTERVAL_MAX_DAYS) return;

    const prevIntervalDays = EARNINGS_INTERVAL_MIN_DAYS + Math.random() * (EARNINGS_INTERVAL_MAX_DAYS - EARNINGS_INTERVAL_MIN_DAYS);
    let backfillDate = new Date(nextEarningsAt.getTime() - prevIntervalDays * 24 * 60 * 60 * 1000);
    if (backfillDate.getTime() > Date.now()) backfillDate = new Date(Date.now() - 24 * 60 * 60 * 1000);

    const fundamental = stock.fundamental || 'stagnant';
    const direction = decideEarningsDirection(fundamental);
    const pct = randomEarningsPct(direction);
    const headline = randomEarningsHeadline(fundamental, direction);
    const oldPrice = stock.price;
    const newPrice = applyNewsPriceChange(oldPrice, pct);

    await sb.from('stock_earnings').insert({
      stock_id: stock.id,
      ...
      old_price: oldPrice,
      new_price: newPrice,
      created_at: backfillDate.toISOString(),
    });
  } catch {
    // 순전히 보여주기용 히스토리라 실패해도 기능엔 지장 없음 — 조용히 무시.
  }
}
...
// 계수를 곱해서, 한쪽으로 완전히 쏠려도(skew=±1) 반대 방향으로 갈 가능성이 남아있게
// 함 — 매수만 계속 일어나는 구조적 특성(공매도가 없어 매도보다 매수가 항상 많음)과 겹쳐서
// 시세가 한쪽으로만 우상향하는 걸 막기 위함입니다. 시뮬레이션으로 확인해보니 0.6에서도
// "그냥 사서 방치"가 "뱃지 보고 제때 사고파는" 가치투자를 거의 따라잡을 만큼 구조적
// 상승압이 셌어서, 0.4로 더 낮췄습니다(아래 평균회귀 강화와 함께 적용).
const SKEW_WEIGHT = 0.4;
...
// 짧은 시간에 확 튄 가격만 눌러줍니다. 위 쏠림 계수 완화와 함께 0.35 -> 0.55로 강화해서,
// 고평가/저평가 뱃지대로 확실하게 행동하는 쪽이 그냥 사서 방치하는 것보다 뚜렷하게
// 유리해지도록 시뮬레이션으로 확인 후 조정했습니다(교육 목적상 "뱃지를 믿고 행동하면
// 이긴다"는 단순한 교훈이 실제로 통하게 하려는 목적).
export const MEAN_REVERSION_WINDOW = 14;
export const MEAN_REVERSION_STRENGTH = 0.55;
...
// 종목별로 실적발표를 최근 N개만 남기고 오래된 건 지웁니다.
export async function pruneStockEarnings(sb, stockId, keep = STOCK_EARNINGS_KEEP) {
  try {
    const { data, error } = await sb
      .from('stock_earnings')
      .select('id')
      ...
      .range(keep, keep + 200);
    if (error || !data || data.length === 0) return;
    await sb.from('stock_earnings').delete().in('id', data.map((r) => r.id));
  } catch {
    // 정리 실패는 무시 — 다음 실적발표 때 다시 시도됩니다.
  }
}

// 실적발표를 stock_earnings에 기록하고, 다음 실적발표일을 새로 잡습니다.
// stock_earnings 테이블/컬럼이 아직 없으면(마이그레이션 전) 그대로 에러를 던져서,
// 호출부가 "이번 틱은 실적발표 없이 평소대로" 처리하도록 합니다.
async function applyEarningsToStock(sb, stock, direction, pct, headline, avgPrice) {
  ...
  const { error: insErr } = await sb.from('stock_earnings').insert({
    stock_id: stock.id,
    stock_name: stock.name,
    headline,
    pct,
  ...
  await pruneStockEarnings(sb, stock.id);

  const { error: updErr } = await sb
    .from('stocks')
    .update({ next_earnings_at: nextEarningsDate().toISOString() })
    .eq('id', stock.id);
...
// sector/fundamental/next_earnings_at 컬럼이 순서대로 없을 수 있는(마이그레이션 전) 상태에
// 대비해 단계적으로 재시도합니다 — 시세 갱신 자체가 통째로 막히면 안 되니까요.
async function fetchStocksForUpdate(sb) {
  let { data, error } = await sb
    .from('stocks')
    .select('id, name, price, sector, fundamental, next_earnings_at')
    .eq('is_active', true);
  if (!error) return data;

  ({ data, error } = await sb.from('stocks').select('id, name, price, sector').eq('is_active', true));
  if (!error) return data.map((s) => ({ ...s, fundamental: null, next_earnings_at: null }));

  const fallback = await sb.from('stocks').select('id, name, price').eq('is_active', true);
  if (fallback.error) throw fallback.error;
  return fallback.data.map((s) => ({ ...s, sector: null, fundamental: null, next_earnings_at: null }));
}

export async function updateStockPrices(sb, { force = false } = {}) {
  const stocks = await fetchStocksForUpdate(sb);
  ...
  for (const stock of stocks) {
    ...
    if (stock.next_earnings_at) {
      const due = new Date(stock.next_earnings_at).getTime() <= Date.now();
      if (due) {
        try {
          const fundamental = stock.fundamental || 'stagnant';
          const newsSkew = await fetchNewsSkew(sb, stock.id);
          const direction = decideEarningsDirection(fundamental, newsSkew);
          const pct = randomEarningsPct(direction);
          const headline = randomEarningsHeadline(fundamental, direction, newsSkew);
          nextPrice = await applyEarningsToStock(sb, stock, direction, pct, headline, avgPrice);
          lastNewsDirectionByStock.set(stock.id, direction);
        } catch {
          // stock_earnings 테이블/컬럼이 아직 없는(마이그레이션 전) 상태 — 이번 틱은
          // 실적발표 없이 평소대로 진행합니다(다음 틱에 다시 시도).
          nextPrice = null;
        }
      }
    } else {
      // 다음 실적발표일이 아직 없는 종목(신규 등록/마이그레이션 직후) — 하나 잡아둡니다.
      try {
        await sb.from('stocks').update({ next_earnings_at: nextEarningsDate().toISOString() }).eq('id', stock.id);
      } catch {
        // next_earnings_at 컬럼이 아직 없으면 무시 — 마이그레이션 후 다시 시도됩니다.
      }
    }
