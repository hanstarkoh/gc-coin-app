...
// 전체(모든 종목 합산) 기준으로 남겨두는 뉴스 개수 — 관리자 대시보드 목록 길이를 제한합니다.
export const STOCK_NEWS_GLOBAL_KEEP = 10;
...
// 전체 뉴스도 최근 N개만 남기고 지웁니다 — 관리자 대시보드 "최근 뉴스" 목록이
// 종목이 여럿이면 계속 길어져서 스크롤이 너무 늘어나는 걸 막기 위함입니다.
export async function pruneStockNewsGlobal(sb, keep = STOCK_NEWS_GLOBAL_KEEP) {
  try {
    const { data, error } = await sb
      .from('stock_news')
      .select('id')
